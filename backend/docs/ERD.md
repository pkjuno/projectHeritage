...
- **조회 로그 증가**: `post_views`는 (글 x 조회자 x 날짜)만큼 쌓입니다. 오래된 행을 지우는
  정리 배치가 필요합니다.
- ~~**신고·차단**~~: 완료. `post_reports` / `comment_reports` / `user_blocks`가
  추가됐습니다. (마이그레이션 `20260903180000`)

  > 신고는 "운영자가 봐줬으면"(공개 조치 요청), 차단은 "나만 안 보고 싶다"(개인 설정)로
  > 답하는 질문이 달라 합치지 않았습니다.
  > 신고가 쌓여도 글은 자동으로 숨겨지지 않습니다 — 자동 숨김은 여러 계정으로
  > 신고를 몰아 남의 글을 지우는 도구가 됩니다.
- ~~**이미지 첨부**~~: 완료. `post_images`가 추가됐습니다. (마이그레이션 `20260903200000`)

  > 본문에 URL을 박지 않고 별도 행으로 둡니다. 저장소를 옮길 때 본문을 고치지 않아도 되고,
  > 글이 지워질 때 어떤 파일을 정리해야 하는지 알 수 있습니다.
  > 다만 **저장소는 아직 로컬 디스크**라 서버가 두 대가 되면 깨집니다.
