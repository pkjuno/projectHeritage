...
| Method | Path | 설명 | 인증 |
| --- | --- | --- | --- |
| POST | `/api/community/posts/:id/report` | 게시글 신고 (body: `reason`, `detail`) | O |
| POST | `/api/community/comments/:id/report` | 댓글 신고 | O |
| GET | `/api/community/me/blocks` | 내가 차단한 회원 | O |
| POST | `/api/community/me/blocks/:userId` | 차단 | O |
| DELETE | `/api/community/me/blocks/:userId` | 차단 해제 | O |
| GET | `/api/community/admin/reports` | 신고 목록 (query: `status`) | O (운영자) |
| PATCH | `/api/community/admin/reports/:targetType/:id` | 신고 처리 (body: `status`, `hide`) | O (운영자) |

신고 사유: `spam` / `abuse` / `adult` / `commercial` / `etc`.
`etc`로 신고하면서 설명이 없으면 400입니다 — 운영자가 무엇을 봐야 할지 알 수 없기 때문입니다.

#### 신고와 차단은 다른 기능입니다

| | 신고 | 차단 |
| --- | --- | --- |
| 답하는 질문 | "운영자가 이 글을 봐줬으면" | "이 사람 글을 나만 안 보고 싶다" |
| 판단 주체 | 운영자 | 본인 |
| 효과 범위 | 공개 조치 (숨김 등) | 내 화면에서만 |

그래서 한 테이블로 합치지 않습니다.

#### 신고가 쌓여도 글은 자동으로 숨겨지지 않습니다

**이것이 이 도메인에서 가장 중요한 규칙입니다.** 자동 숨김을 넣으면
계정을 여러 개 만들어 신고를 몰아 남의 글을 지우는 도구가 됩니다.
숨길지 말지는 운영자가 `hide: true`로 명시할 때만 일어납니다.

같은 글에 대한 신고는 **한 번에 함께 처리**됩니다. 하나씩 처리하게 두면
같은 글에 대한 신고 10건이 목록에 10번 남습니다.

처리 시 `handledBy`와 `handledAt`을 기록합니다. 누가 무엇을 판단했는지
남지 않으면 이력의 의미가 없습니다.

#### 차단의 범위

차단하면 그 회원의 **글과 댓글이 모두** 내 화면에서 사라집니다.
글만 가리고 댓글이 보이면 차단이 아닙니다.

- 차단은 삭제가 아니므로 원본은 그대로 두고 조회하는 쪽에서만 걸러냅니다.
  다른 회원과 비회원에게는 그대로 보입니다.
- 차단당한 사람은 자기가 차단됐다는 사실을 알 수 없습니다.
  알려주면 차단이 곧 상대에게 보내는 메시지가 되어 갈등을 키웁니다.
- **운영자는 차단할 수 없습니다.** 차단하면 공지사항이 보이지 않게 됩니다.

### 게시글 이미지 첨부

글 작성 요청을 `multipart/form-data`로 보내고 `images` 필드에 파일을 담습니다.
이미지가 없으면 평소처럼 JSON으로 보내면 됩니다.

```bash
curl -X POST /api/community/posts -H "Authorization: Bearer $TOKEN" \
  -F category=review -F title=후기 -F content=본문 \
  -F images=@a.jpg -F images=@b.jpg
```

- 한 글에 **최대 5장** (`UPLOAD_MAX_POST_IMAGES`), 장당 5MB
- 허용 형식: jpg / png / webp / gif
- **이미지는 상세 응답에만 담깁니다.** 목록에서 본문을 `preview`로 줄인 것과 같은 이유로,
  20건마다 이미지 배열이 붙으면 첫 화면 응답이 불필요하게 커집니다.

#### 본문에 URL을 박지 않고 별도 테이블을 쓰는 이유

`post_images` 행으로 관리합니다. 본문에 URL을 박아 넣으면

- 저장소를 옮길 때(로컬 → S3) 모든 글의 본문을 찾아 고쳐야 하고,
- 글이 지워질 때 어떤 파일을 정리해야 하는지 알 수 없습니다.

#### 원본 파일명은 경로로 쓰지 않습니다

저장 파일명은 UUID로 새로 만들고, 원본 파일명은 표시용으로만 `original_name`에 남깁니다.
클라이언트가 보낸 이름을 그대로 경로에 쓰면 `../../evil.png` 같은 값으로
상위 디렉터리에 접근할 수 있습니다. (테스트가 이 경우를 확인합니다)

#### 글을 지우면 파일도 지웁니다

글 자체는 `status='deleted'`로 남지만 **이미지 파일은 실제로 삭제**합니다.
행만 지우고 파일을 두면 URL을 아는 사람은 지운 글의 사진을 계속 볼 수 있고,
디스크에는 아무도 참조하지 않는 파일이 영원히 쌓입니다.

### ⚠️ 파일 저장소는 아직 로컬 디스크입니다

프로필 이미지와 게시글 이미지 모두 서버의 로컬 디스크에 저장됩니다.
**이 방식은 서버가 두 대가 되는 순간 깨집니다** — A 서버에 올린 파일을
B 서버가 서빙할 수 없기 때문입니다.

그때를 대비해 파일을 다루는 코드는 `services/imageStorage.service.js` 한 곳에만 뒀습니다.
S3로 옮길 때 고쳐야 할 곳은 이 파일뿐이고, `save`/`remove` 시그니처만 지키면 됩니다.
(`post_images.url`은 이미 전체 URL이 들어갈 수 있는 길이입니다)

### 알려진 한계

- **검색**: `LIKE '%키워드%'`라 인덱스를 타지 못합니다.
  한글 부분어 검색을 인덱스로 처리하려면 **MySQL 5.7+의 ngram 파서**가 필요합니다
  (`ALTER TABLE posts ADD FULLTEXT INDEX ... WITH PARSER ngram`).

  > 아직 넣지 않았습니다. 이 저장소의 검증 환경은 MariaDB인데
  > MariaDB에는 ngram 파서가 없어(`Function 'ngram' is not defined`)
  > 마이그레이션이 실행되지 않고 드리프트 테스트도 깨집니다.
  > 파서 없는 일반 FULLTEXT는 한글 부분어(`부산불꽃축제`에서 `불꽃`)를 찾지 못해
  > 지금의 LIKE보다 오히려 나쁩니다.
  >
  > **MySQL로 운영이 확정되면** 그때 넣는 것이 맞습니다.
  > 검증할 수 없는 마이그레이션을 미리 넣어두면 테스트만 깨진 채로 남습니다.
...
- **신고 목록 페이지네이션**: 게시글 신고와 댓글 신고를 애플리케이션에서 합친 뒤
  자르므로 DB 페이지네이션을 쓰지 않습니다. 처리 대기 신고는 많아야 수백 건이라
  지금은 문제되지 않지만, 규모가 커지면 UNION 쿼리로 옮겨야 합니다.
- 이미지 첨부는 아직 없습니다. (`docs/COMMUNITY_PLAN.md` 9장 참고)
